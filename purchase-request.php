...
if (isset($connect) && !$connect->connect_error) {
    $result = $connect->query('SELECT * FROM purchase_request ORDER BY pr_date DESC');
    if ($result) {
...
$totalQuantity = 0;
$statusCounts = ['Pending' => 0, 'Approved' => 0, 'Rejected' => 0, 'Completed' => 0];
...
function statusBadge(string $status): string {
    $map = [
        'Pending'   => ['bg-warning/10 text-warning', 'clock'],
        'Approved'  => ['bg-success/10 text-success', 'check-circle-2'],
        'Rejected'  => ['bg-danger/10 text-danger', 'x-circle'],
        'Completed' => ['bg-info/10 text-info', 'package-check'],
    ];
    $key = ucfirst(strtolower($status));
    [$classes, $icon] = $map[$key] ?? ['bg-default-200 text-default-600', 'circle-dot'];
    return '<span class="py-0.5 px-2.5 inline-flex items-center gap-x-1 text-xs font-medium ' . $classes . ' rounded">'
...
<head>
    <meta charset="utf-8">
    <title>Purchasing Request | PICS</title>
...
<div class="topbar-item hs-dropdown relative inline-flex">
            ...
            <main>
                ...
                <div class="flex items-center md:justify-between flex-wrap gap-2 mb-4 print:hidden">
                    <h4 class="text-default-900 text-lg font-semibold">Purchasing Request</h4>
                    ...
                    <div class="md:flex hidden items-center gap-2 text-sm font-semibold">
                        ...
                        <i class="iconify tabler--chevron-right text-sm flex-shrink-0 text-default-500 rtl:rotate-180"></i>
                        <a href="#" class="text-sm font-medium text-default-700" aria-current="page">Purchasing Request</a>
                    </div>
                </div>
                <!-- Page Title End -->
                ...
                <div class="grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-5 mb-5">
                        ...
                        <div class="card-body">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <h4 class="text-xl font-semibold text-default-900 mt-1.5"><?= $totalRequests ?></h4>
                                </div>
                                <div class="size-11 rounded-lg bg-primary/10 flex items-center justify-center">
                                    <i data-lucide="shopping-cart" class="size-5 text-primary"></i>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-default-500 text-sm">Pending</p>
                                    <h4 class="text-xl font-semibold text-default-900 mt-1.5"><?= $statusCounts['Pending'] ?></h4>
                                </div>
                        ...
                        <div class="card-body">
                            <div class="flex items-center justify-between">
                                ...
                                <div class="size-11 rounded-lg bg-info/10 flex items-center justify-center">
                                    <i data-lucide="package" class="size-5 text-info"></i>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card">
                    ...
                    <div class="card-header">
                        <div class="md:flex items-center md:space-y-0 space-y-4 gap-3">
                            <div class="relative">
                                <input type="search" id="table-search" class="form-input form-input-sm ps-9" placeholder="Search PR, items or requester">
                        ...
                        <div class="flex gap-2 items-center flex-wrap">
                            <span class="text-sm text-default-500"><?= $totalRequests ?> request<?= $totalRequests === 1 ? '' : 's' ?> found</span>
                        ...
                        <div class="overflow-x-auto">
                            <div class="min-w-full inline-block align-middle">
                                <div class="overflow-hidden">
                                    <table id="pr-table" class="min-w-full divide-y divide-default-200">
                                        <thead class="bg-default-150">
                                            <tr class="text-sm font-normal text-default-700">
                                                <th scope="col" class="px-3.5 py-3 text-start">PR Number</th>
                                                <th scope="col" class="px-3.5 py-3 text-start">Date</th>
                                                <th scope="col" class="px-3.5 py-3 text-start">Request By</th>
                                                <th scope="col" class="px-3.5 py-3 text-start">Department</th>
                                            ...
                                            <?php if (empty($records)): ?>
                                                <tr>
                                                    <td colspan="7" class="px-3.5 py-16 text-center text-default-500">
                                                        <i data-lucide="shopping-cart" class="size-10 mx-auto mb-3 text-default-300"></i>
                                                        <p class="font-medium">No purchase requests found yet.</p>
                                                    </td>
                                                </tr>
                                            <?php else: foreach ($records as $record): ?>
                                                <tr class="text-default-800 font-normal text-sm" data-status="<?= htmlspecialchars(trim($record['status']), ENT_QUOTES, 'UTF-8') ?>">
                                                    <td class="px-3.5 py-3 text-sm text-primary font-medium"><?= htmlspecialchars($record['pr_number'], ENT_QUOTES, 'UTF-8') ?></td>
                                                    <td class="py-3 px-3.5 text-default-600"><?= htmlspecialchars(date('M d, Y', strtotime($record['pr_date'])), ENT_QUOTES, 'UTF-8') ?></td>
                                                    <td class="py-3 px-3.5 text-default-800 font-medium"><?= htmlspecialchars($record['requested_by'], ENT_QUOTES, 'UTF-8') ?></td>
                                                    <td class="py-3 px-3.5 text-default-600"><?= htmlspecialchars($record['department'], ENT_QUOTES, 'UTF-8') ?></td>
                                                    ...
                                                    <td class="px-3.5 py-3"><?= statusBadge($record['status']) ?></td>
                                                </tr>
...
<div class="lg:px-8 px-6 w-full flex md:justify-between justify-center gap-4">
    ...
    <script>
        (function () {
            ...
            const rows = document.querySelectorAll('#pr-table tbody tr[data-status]');
